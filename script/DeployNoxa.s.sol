...
///   The factory / locker / fee-router / NOXA token are immutable by design — a bug
///   is fixed by deploying a fresh generation and re-pointing the frontend + indexer.
///
///   The NOXA token's full supply is minted to the deployer; distribution (e.g. a
///   separate Merkle airdrop or seeding the buy-burner) is handled out of band.
///
///   Config below is MIRRORED from the original live factory 0xD9eC2db5…FCcB (read on-chain),
///   with two deployment overrides: quorum 25% and a 50/50 protocol/creator fee split.
///
...
///     - cto.transferOwnership(multisig)             (election parameters)
///     - factory.transferOwnership(multisig); locker.transferOwnership(multisig)
///     - feeRouter.transferOwnership(multisig); feeRouter.setTreasury(multisig)
contract DeployNoxa {
    ...
    // ---- NOXA token ----
    string constant NOXA_LOGO = "https://fun.noxa.eth.limo/favicon.png";
    string constant NOXA_DESCRIPTION = "noxa2.finance protocol token";
    string constant NOXA_WEBSITE = "https://noxa2.fi";
    ...
    function run() external {
        ...
        // Deployer owns everything and receives protocol fees; hand off to a multisig later.
        address owner = deployer;
        address treasury = vm.envOr("NOXA_TREASURY", deployer);

        vm.startBroadcast();

        // 1. NOXA token — fixed 1M supply minted to the deployer, with on-chain metadata getters.
        Noxa2Token.Socials memory socials = Noxa2Token.Socials("", "", "", NOXA_WEBSITE, "");
        Noxa2Token noxa = new Noxa2Token(deployer, NOXA_LOGO, NOXA_DESCRIPTION, socials);

        // 2. FeeRouter — 50/50 protocol/creator split on both token and pair fees.
        FeeRouter feeRouter = new FeeRouter(WETH, treasury, PROTOCOL_SHARE_BPS, TOKEN_TREASURY_SHARE_BPS);

        // 3. Locker holds LP NFTs permanently; wire it to the router (one-time).
        LauncherLocker locker = new LauncherLocker(payable(address(feeRouter)));
        feeRouter.setLocker(address(locker));

        // 4. Factory (launches disabled until fully wired); wire the locker back to it (one-time).
        LaunchFactory factory = new LaunchFactory(address(locker), LAUNCH_FEE, false);
        locker.setFactory(address(factory));

        // 5. CTOFund behind a transparent proxy — impl deployed, ProxyAdmin auto-created (owner-owned),
        ...
        // 6. Bind the fund to the factory (one-time) and set the 25% quorum.
